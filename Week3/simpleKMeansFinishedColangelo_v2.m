...
normPoints = points - repmat(minPoints,  sizePoints(1), 1)
normPoints(:, 1) = normPoints(:, 1) ./ rangePoints(1)
normPoints(:, 2) = normPoints(:, 2) ./ rangePoints(2)
...
normCentroids = centroids - repmat(minPoints, sizeCentroids(1), 1)
normCentroids(:, 1) = normCentroids(:, 1) ./ rangePoints(1)
normCentroids(:, 2) = normCentroids(:, 2) ./ rangePoints(2)
...
for (iter1 = 1:20)
    ...
    clusterID = simpleAssignToCentroids(points, normCentroids);
    ...
    normCentroids = simpleDetermineCentroids(points, clusterID, size(normCentroids, 1));
...
% of the dimension
% Missing code:
denormCentroids(:, 1) = normCentroids(:, 1) .* rangePoints(1)
denormCentroids(:, 2) = normCentroids(:, 2) .* rangePoints(2)
denormCentroids = denormCentroids + repmat(minPoints, sizeCentroids(1), 1)
